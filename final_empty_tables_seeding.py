#!/usr/bin/env python3
"""
Final Empty Tables Seeding Script
Properly handles UUIDs, migrates existing data, and populates all empty tables
"""
            ...
            if not col_info['nullable'] and col_name != referenced_column:
                if 'UUID' in col_info['type']:
                    ...
                    if col_name in ['title', 'name']:
                        data[col_name] = f"Ref {random.randint(1, 999)}"
                    elif col_name in ['subject', 'grade_level']:
                        data[col_name] = f"G{random.randint(1, 12)}"
                    else:
                        data[col_name] = f"ref_{random.randint(1, 99)}"
...
def generate_final_data(session, table_name, schema, existing_data_cache):
    """Generate final data that properly handles all requirements"""
    data = []
...
def insert_final_data(session, table_name, data):
    """Insert data with proper UUID handling"""
    if not data:
        return 0
    ...
    try:
        ...
        # Try upsert first
        if pk_column and not pk_is_uuid:  # Only use upsert for non-UUID primary keys
...
def final_empty_tables_seeding():
    """Main function to seed empty tables with final robust logic"""
    print("🚀 Starting Final Empty Tables Seeding...")
    ...
    try:
        ...
        print(f"\n🎉 Final seeding completed! Seeded {seeded_count} empty tables successfully!")
