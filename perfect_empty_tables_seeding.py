#!/usr/bin/env python3
"""
Perfect Empty Tables Seeding Script
Matches the exact pattern used in working seeding scripts
"""
...
def generate_perfect_data(session, table_name, schema, existing_data_cache):
    """Generate perfect data that properly handles all requirements"""
    data = []
...
def insert_perfect_data(session, table_name, data):
...
def perfect_empty_tables_seeding():
    """Main function to seed empty tables with perfect logic"""
    print("🚀 Starting Perfect Empty Tables Seeding...")
    ...
    try:
        ...
        for table in empty_tables:
            try:
                ...
                data = generate_perfect_data(session, table, schema, existing_data_cache)
                
                if data:
                    count = insert_perfect_data(session, table, data)
                    if count > 0:
        ...
        print(f"\n🎉 Perfect seeding completed! Seeded {seeded_count} empty tables successfully!")
