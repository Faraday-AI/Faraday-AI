...
def generate_sample_data(num_students: int = 100, num_activities: int = 20) -> Dict[str, pd.DataFrame]:
    ...
    os.makedirs('app/models/utils', exist_ok=True)
...
def generate_models():
    ...
    for model_dir in ['app/models/utils', 'app/models']:
        joblib.dump(model, os.path.join(model_dir, 'performance_prediction.joblib'))
