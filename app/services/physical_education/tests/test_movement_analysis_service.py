...
async def service(db_session, redis_client):
    ...
    service.circuit_breaker.can_execute = AsyncMock(return_value=True)
...
async def test_circuit_breaker(service, sample_movement_data):
    ...
    service.circuit_breaker.can_execute.return_value = False
...
async def test_concurrent_operations(service, sample_movement_data):
    ...
    def mock_track_metrics(endpoint: str = None):
        def decorator(func):
            ...
            async def wrapper(*args, **kwargs):
                # Extract student_id from kwargs if present
    ...
    with patch('app.core.monitoring.track_metrics', side_effect=mock_track_metrics), \
         patch('app.services.physical_education.services.movement_analysis_service.cache_result', side_effect=mock_cache_result):
        
        async def simulate_analysis(student_id: str, activity_id: int):
            try:
                return await service_instance.create_movement_analysis(
