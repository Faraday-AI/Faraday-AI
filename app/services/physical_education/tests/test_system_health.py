...
@pytest.mark.asyncio
async def test_system_health_monitoring(service, redis_client, rate_limiter, circuit_breaker, cache_monitor):
    ...
    assert health["latency"] < 0.1  # Should be under 100ms
    assert health["hit_rate"] >= 0
    assert health["error_rate"] == 0

@pytest.mark.asyncio
async def test_system_health_under_load(service, redis_client, rate_limiter, circuit_breaker, cache_monitor):
    ...
    health = await cache_monitor.check_health()
    assert health["status"] == "healthy"
    assert health["latency"] < 0.2  # Should be under 200ms under load
    assert health["hit_rate"] >= 0
    assert health["error_rate"] == 0

@pytest.mark.asyncio
async def test_system_health_error_handling(service, redis_client, rate_limiter, circuit_breaker, cache_monitor):
...
@pytest.mark.asyncio
async def test_system_health_recovery(service, redis_client, rate_limiter, circuit_breaker, cache_monitor):
    ...
    health = await cache_monitor.check_health()
    assert health["status"] == "healthy"
    assert health["error_rate"] < 0.5  # Error rate should be reduced
    assert health["latency"] < 0.1  # Should be back to normal latency

@pytest.mark.asyncio
async def test_system_health_metrics(service, redis_client, rate_limiter, circuit_breaker, cache_monitor):
    ...
    async def run_operations():
        for i in range(10):
            @rate_limiter
            @circuit_breaker
            async def metric_operation():
                start_time = time.time()
                await cache_monitor.track_operation("set", start_time)
                await redis_client.set(f"metric_test_{i}", json.dumps({"data": f"test_{i}"}))
                await cache_monitor.track_operation("get", start_time)
                return await redis_client.get(f"metric_test_{i}")
            
            await metric_operation(client_id=f"metric_client_{i}")
