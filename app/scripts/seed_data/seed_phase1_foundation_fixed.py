...
def seed_user_profiles(session):
    ...
    # Check if already has data, migrating data...
    result = session.execute(text("SELECT COUNT(*) FROM user_profiles"))
    if result.scalar() > 0:
        print("    ✅ user_profiles already has data, migrating data...")
        return
...
def seed_user_roles(session):
    ...
    if result.scalar() > 0:
        print("    ✅ user_roles already has data, migrating data...")
        return
...
def seed_user_sessions(session):
    ...
    if result.scalar() > 0:
        print("    ✅ user_sessions already has data, migrating data...")
        return
...
def seed_user_activities(session):
    ...
    if result.scalar() > 0:
        print("    ✅ user_activities already has data, migrating data...")
        return
...
def seed_user_behaviors(session):
    ...
    if result.scalar() > 0:
        print("    ✅ user_behaviors already has data, migrating data...")
        return
...
def seed_user_engagements(session):
    ...
    if result.scalar() > 0:
        print("    ✅ user_engagements already has data, migrating data...")
        return
...
def seed_user_insights(session):
    ...
    if result.scalar() > 0:
        print("    ✅ user_insights already has data, migrating data...")
        return
...
def seed_user_trends(session):
    ...
    if result.scalar() > 0:
        print("    ✅ user_trends already has data, migrating data...")
        return
...
def seed_user_predictions(session):
    ...
    if result.scalar() > 0:
        print("    ✅ user_predictions already has data, migrating data...")
        return
...
def seed_user_comparisons(session):
    ...
    if result.scalar() > 0:
        print("    ✅ user_comparisons already has data, migrating data...")
        return
...
def seed_access_control_system(session):
    ...
    # Check if access_control_roles already has data, migrating data...
    result = session.execute(text("SELECT COUNT(*) FROM access_control_roles"))
    if result.scalar() > 0:
        print("      ✅ access_control_roles already has data, migrating data...")
    else:
        # Create basic roles
        roles_data = [
...
def seed_access_control_permissions(session):
    ...
    if result.scalar() > 0:
        print("        ✅ access_control_permissions already has data, migrating data...")
        return
...
def seed_role_permissions(session):
    ...
    if result.scalar() > 0:
        print("        ✅ access_control_role_permissions already has data, migrating data...")
        return
...
def seed_permissions_table(session):
    ...
    if result.scalar() > 0:
        print("        ✅ permissions already has data, migrating data..., migrating data...")
        return
...
def seed_role_permissions_table(session):
    ...
    if result.scalar() > 0:
        print("        ✅ role_permissions already has data, migrating data...")
        return
...
def seed_permission_overrides(session):
    ...
    if result.scalar() > 0:
        print("        ✅ permission_overrides already has data, migrating data...")
        return
...
def seed_feedback_user_tool_settings(session):
    ...
    if result.scalar() > 0:
        print("        ✅ feedback_user_tool_settings already has data, migrating data...")
        return
...
def seed_user_management_voice_preferences(session):
    ...
    if result.scalar() > 0:
        print("        ✅ user_management_voice_preferences already has data, migrating data...")
        return
...
def seed_user_management_preferences(session):
    ...
    if result.scalar() > 0:
        print("        ✅ user_management_preferences already has data, migrating data...")
        return
...
def seed_user_management_user_organizations(session):
    ...
    if result.scalar() > 0:
        print("        ✅ user_management_user_organizations already has data, migrating data...")
        return
...
def seed_user_tool_settings(session):
    ...
    if result.scalar() > 0:
        print("        ✅ user_tool_settings already has data, migrating data...")
        return
...
def seed_user_tools(session):
    ...
    if result.scalar() > 0:
        print("        ✅ user_tools already has data, migrating data...")
        return
...
def seed_user_preference_categories(session):
    ...
    if result.scalar() > 0:
        print("        ✅ user_preference_categories already has data, migrating data...")
        return
...
def seed_user_preference_templates(session):
    ...
    if result.scalar() > 0:
        print("        ✅ user_preference_templates already has data, migrating data...")
        return
...
def seed_user_preference_template_assignments(session):
    ...
    if result.scalar() > 0:
        print("        ✅ user_preference_template_assignments already has data, migrating data...")
        return
...
def seed_user_recommendations(session):
    ...
    if result.scalar() > 0:
        print("        ✅ user_recommendations already has data, migrating data...")
        return
...
def seed_role_hierarchy(session):
    ...
    if result.scalar() > 0:
        print("        ✅ role_hierarchy already has data, migrating data...")
        return
...
def seed_role_templates(session):
    ...
    if result.scalar() > 0:
        print("        ✅ role_templates already has data, migrating data...")
        return
...
def seed_security_preferences(session):
    ...
    if result.scalar() > 0:
        print("        ✅ security_preferences already has data, migrating data...")
        return
...
def seed_sessions(session):
    ...
    if result.scalar() > 0:
        print("        ✅ sessions already has data, migrating data...")
        return
...
def seed_shared_contexts(session):
    ...
    if result.scalar() > 0:
        print("        ✅ shared_contexts already has data, migrating data...")
        return
...
def seed_tool_assignments(session):
    ...
    if result.scalar() > 0:
        print("        ✅ tool_assignments already has data, migrating data...")
        return
...
def seed_voice_templates(session):
    ...
    if result.scalar() > 0:
        print("        ✅ voice_templates already has data, migrating data...")
        return
...
def seed_voices(session):
    ...
    if result.scalar() > 0:
        print("        ✅ voices already has data, migrating data...")
        return
...
def seed_avatar_templates(session):
    ...
    if result.scalar() > 0:
        print("        ✅ avatar_templates already has data, migrating data...")
        return
...
def seed_avatar_customizations(session):
    ...
    if result.scalar() > 0:
        print("        ✅ avatar_customizations already has data, migrating data...")
        return
...
def seed_user_avatars(session):
    ...
    if result.scalar() > 0:
        print("        ✅ user_avatars already has data, migrating data...")
        return
...
def seed_user_avatar_customizations(session):
    ...
    if result.scalar() > 0:
        print("        ✅ user_avatar_customizations already has data, migrating data...")
        return
...
def seed_student_avatar_customizations(session):
    ...
    if result.scalar() > 0:
        print("        ✅ student_avatar_customizations already has data, migrating data...")
        return
...
def seed_security_logs(session):
    ...
    if result.scalar() > 0:
        print("        ✅ security_logs already has data, migrating data...")
        return
...
def seed_user_role_assignments(session):
    ...
    if result.scalar() > 0:
        print("        ✅ access_control_user_roles already has data, migrating data...")
        return
...
def seed_notifications_system(session):
    ...
    try:
        ...
        if result.scalar() > 0:
            print("      ✅ dashboard_notification_channels already has data, migrating data...")
            return
...
def seed_logging_system(session):
    ...
    try:
        ...
        if result.scalar() > 0:
            print("      ✅ activity_logs already has data, migrating data...")
            return
...
def seed_monitoring_system(session):
    ...
    try:
        ...
        if result.scalar() > 0:
            print("      ✅ audit_logs already has data, migrating data...")
            return
...
def seed_system_utilities(session):
    ...
    try:
        ...
        if result.scalar() > 0:
            print("      ✅ organization_settings already has data, migrating data...")
        else:
            # Get organization_id
    ...
    try:
        ...
        if result.scalar() > 0:
            print("      ✅ project_settings already has data, migrating data...")
        else:
...
def seed_avatar_system(session):
    ...
    try:
        ...
        if roles_count > 0:
            print("    ✅ Roles table already has data, migrating data...")
        else:
    ...
    try:
        ...
        if avatar_count > 0:
            print("    ✅ Avatars table already has data, migrating data...")
        else:
