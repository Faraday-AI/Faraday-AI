...
Seeds 35 tables for advanced PE features with correct schemas
"""
...
def seed_phase8_advanced_pe(session: Session) -> Dict[str, int]:
    ...
    # physical_education_class_routines (FIXED SCHEMA)
    try:
        additional_routines = []
    ...
    # adapted_routines (FIXED SCHEMA - use physical_education_classes)
    try:
        additional_routines = []
    ...
    # workout_exercises (FIXED SCHEMA - use 'order' not 'order_in_workout')
    try:
    ...
    # physical_education_safety_alerts (FIXED SCHEMA - no student_id, different columns)
    try:
        additional_alerts = []
        for i in range(50):  # Create 50 safety alerts
            ...
            additional_alerts.append({
                ...
                'activity_id': activity_id,
                'equipment_id': random.randint(1, 100) if random.choice([True, False]) else None,
    ...
    # physical_education_student_fitness_goal_progress (FIXED SCHEMA - no student_id, different columns)
    try:
        additional_progress = []
    ...
    # workout_performances (FIXED SCHEMA - use performance_date not completed_at)
    try:
    ...
    # workout_sessions (FIXED SCHEMA - use start_time/end_time not session_date)
    try:
        additional_sessions = []
