#!/usr/bin/env python
"""
Phase 8: Advanced Physical Education & Adaptations - Working Version
Seeds 35 tables for advanced PE features with all schema fixes applied
"""
...
def seed_phase8_advanced_pe(session: Session) -> Dict[str, int]:
    ...
    # physical_education_attendance (migrate from students) - remove class_id column
    try:
    ...
    # adapted_activity_plans (migrate from students) - fix status enum values
    try:
        additional_plans = []
        for student_id in ids['student_ids'][:50]:  # Create plans for first 50 students
            ...
            additional_plans.append({
                ...
                'status': random.choice(['ACTIVE', 'INACTIVE', 'PENDING', 'APPROVED', 'REJECTED']),  # Fixed enum values
                'is_active': True,
                'created_at': start_date,
    ...
    # adapted_routines (use educational_classes instead of physical_education_classes) - fix status enum values
    try:
        additional_routines = []
    ...
    # student_activity_adaptations (migrate from students + activities) - remove adaptation_level column
    try:
        additional_adaptations = []
        for student_id in ids['student_ids'][:50]:  # Create adaptations for first 50 students
    ...
    # workout_exercises (migrate from workouts + exercises) - fix column names
    try:
        ...
        for i in range(50):  # Create 50 workout exercises
            ...
            additional_workout_exercises.append({
                ...
                'reps': random.randint(5, 20),
                'sets': random.randint(1, 5),
                'rest_seconds': random.randint(30, 120),  # Remove duration_seconds column
                'created_at': datetime.now() - timedelta(days=random.randint(1, 30)),
                'updated_at': datetime.now() - timedelta(days=random.randint(1, 30)),
    ...
    # physical_education_safety_alerts - remove created_at column
    try:
        additional_alerts = []
    ...
    # workout_performances (migrate from students + workouts) - remove total_exercises column
    try:
...
if __name__ == "__main__":
    ...
    print('🚀 TESTING PHASE 8 - WORKING VERSION')
    print('=' * 60)
