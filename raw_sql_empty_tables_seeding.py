#!/usr/bin/env python3
"""
Raw SQL Empty Tables Seeding Script
Uses raw SQL like the working scripts to handle UUIDs properly
"""
...
def generate_raw_sql_data(session, table_name, schema, existing_data_cache):
    """Generate data for raw SQL insertion"""
    data = []
...
def insert_raw_sql_data(session, table_name, data):
    """Insert data using raw SQL like the working scripts"""
    if not data:
        return 0
    ...
    try:
        ...
        # Build the INSERT query using raw SQL like working scripts
...
def raw_sql_empty_tables_seeding():
    """Main function to seed empty tables with raw SQL"""
    print("🚀 Starting Raw SQL Empty Tables Seeding...")
    ...
    try:
        ...
        for table in empty_tables:
            try:
                ...
                # Generate raw SQL data
                data = generate_raw_sql_data(session, table, schema, existing_data_cache)
                
                if data:
                    count = insert_raw_sql_data(session, table, data)
                    if count > 0:
        ...
        print(f"\n🎉 Raw SQL seeding completed! Seeded {seeded_count} empty tables successfully!")
