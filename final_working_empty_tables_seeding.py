#!/usr/bin/env python3
"""
Final Working Empty Tables Seeding Script
Uses the exact same pattern as the working seeding scripts
"""
...
def generate_final_data(session, table_name, schema, existing_data_cache):
    """Generate data for final insertion"""
    data = []
                ...
                if not is_nullable or random.random() > 0.1:  # 90% chance to populate non-nullable
                    if 'UUID' in col_type:
                        ...
                        if 'NUMERIC(3,2)' in col_type:
                            record[col_name] = round(random.uniform(0.0, 9.99), 2)
                        elif 'NUMERIC(5,2)' in col_type:
                            record[col_name] = round(random.uniform(0.0, 999.99), 2)
                        else:
...
def insert_final_data(session, table_name, data):
    """Insert data using the exact pattern from working scripts"""
    if not data:
        return 0
    ...
    try:
        ...
        # Build the INSERT query using the exact pattern from working scripts
        ...
        # Use raw SQL like the working scripts
        base_query = f"INSERT INTO {table_name} ({columns_str}) VALUES ("
        
        # Add gen_random_uuid() for UUID primary keys
        if pk_is_uuid and pk_column and pk_column not in columns:
            base_query += "gen_random_uuid(), "
        
        # Add placeholders for the data columns
        placeholders = ', '.join([f':{col}' for col in columns])
        base_query += f"{placeholders})"
...
def final_working_empty_tables_seeding():
    """Main function to seed empty tables with final working logic"""
    print("🚀 Starting Final Working Empty Tables Seeding...")
    ...
    try:
        ...
        for table in empty_tables:
            try:
                ...
                # Generate final data
                data = generate_final_data(session, table, schema, existing_data_cache)
                
                if data:
                    count = insert_final_data(session, table, data)
                    if count > 0:
        ...
        print(f"\n🎉 Final working seeding completed! Seeded {seeded_count} empty tables successfully!")
...
if __name__ == "__main__":
    final_working_empty_tables_seeding()
