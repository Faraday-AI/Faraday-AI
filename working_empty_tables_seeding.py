#!/usr/bin/env python3
"""
Working Empty Tables Seeding Script
...
def generate_working_data(session, table_name, schema, existing_data_cache):
    """Generate working data that properly handles all requirements"""
    data = []
...
def insert_working_data(session, table_name, data):
    ...
    try:
        ...
        # Use the exact pattern from working scripts
...
def working_empty_tables_seeding():
    """Main function to seed empty tables with working logic"""
    print("🚀 Starting Working Empty Tables Seeding...")
    ...
    try:
        ...
        for table in empty_tables:
            try:
                ...
                data = generate_working_data(session, table, schema, existing_data_cache)
                
                if data:
                    count = insert_working_data(session, table, data)
                    if count > 0:
        ...
        print(f"\n🎉 Working seeding completed! Seeded {seeded_count} empty tables successfully!")
